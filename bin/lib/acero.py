...
class ResponseViewerWindow(BaseAssistantWindow):
    ...
    def _add_input_and_send_bar(self):
        '''Adds an input text area and a 'Send' button.'''
        ...
        self.user_input_field.returnPressed.connect(self._send_input_to_ai)
    ...
    def _send_input_to_ai(self):
        '''Sends the user input to stdout with a special prefix for the AI to pick up.'''
        user_text = self.user_input_field.text().strip()
        if user_text:
...
def main(args):
    ...
    sample_ai_response = ''' Acero Window. \n '''
