...
def set_model_params(params):
    ...
    try:
        model_params = {"temperature": 0.9}  #--cc --sgmParámetros por defecto
        if params:
            for param in params:
...
#            print("End Audio Parser")
        ...
        if command == "--ctm" or command == "--context-mode":
            ...
            if context_mode == "fast":
                context_mode = False
                conversation_context += "\n Contexto mode Fue puesto a False, el contexto solo guarda las preguntas, debes inferir retroactivamente cual fue tu respuesta anterior más probable  con este modo activo.\n"
            else:
                context_mode = "fast"
                conversation_context += "\n Context Mode se ha puesto activado en modo fast, desde este punto se guardará en el contexto preguntas del usuario y tus respuestas. Cuando detectes respuestas irrelevantes para el contexto  en un rango 20d0-100 sugiere al usuario eliminar esos fragmentos.\n"
        ...
        if command == "--arc" or command == "--add-response-context":
            if last_response:
                # Asegurándose de que conversation_context sea accesible y modificable.
                # Se asumirá que conversation_context es un diccionario para añadir la respuesta.
                # Si el sistema de contexto es diferente, esta línea podría necesitar ajuste manual.
                conversation_context += last_response
                print("La última respuesta de la IA ha sido añadida al contexto.")
            else:
                print("No hay una última respuesta de la IA para añadir al contexto.")
            return
...
conversation_context += HELO+"\n  Comandos de ayuda, esta serio de funcionalidesdes de comandos se llamarán Glyphs : "+str(commands_map)+"\n--\n"
